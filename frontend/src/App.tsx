...
function ProfileViewerPage() {
  ...
  return (
    <div className="page profile-viewer-page">
      ...
      <ProfilePresentation userProfile={bundle.user_profile.content} masterCv={bundle.master_cv_profile.content} policy={bundle.policy.content} mode="approved" onApproveClaim={async (claimId) => { const next = await mutate<ApprovedProfileBundle>(`/profile/claims/${encodeURIComponent(claimId)}/approve`, "POST"); setBundle(next); setNotice("Claim approved. Your confirmation is now recorded in the profile."); }} />
    </div>
  );
}

function ProfilePresentation({ userProfile, masterCv, policy, mode, onApproveClaim }: { userProfile: Record<string, any>; masterCv: Record<string, any>; policy: Record<string, any>; mode: "approved" | "candidate"; onApproveClaim?: (claimId: string) => Promise<void> }) {
  ...
  return (
    <div className={`profile-presentation ${mode}`}>
      ...
      <section className="profile-section"><SectionHeading eyebrow="Approved evidence" title="Career claims your team can use" body="Claims are grouped for quick reading. Tailoring status records which claims you have explicitly confirmed." />{Object.keys(claimGroups).length ? <div className="claim-groups">{Object.entries(claimGroups).map(([category, items]) => <div className="claim-group" key={category}><h3>{human(category)}</h3>{(items as any[]).map((claim: any) => <ProfileClaimCard key={claim.claim_id} claim={claim} canApprove={mode === "approved" && Boolean(onApproveClaim)} confirming={confirmingClaim === claim.claim_id} approving={approvingClaim === claim.claim_id} error={confirmingClaim === claim.claim_id ? approvalError : ""} onBeginApproval={() => { setConfirmingClaim(claim.claim_id); setApprovalError(""); }} onCancel={() => setConfirmingClaim("")} onConfirm={async () => { if (!onApproveClaim) return; setApprovingClaim(claim.claim_id); setApprovalError(""); try { await onApproveClaim(claim.claim_id); setConfirmingClaim(""); } catch (cause) { setApprovalError(messageOf(cause)); } finally { setApprovingClaim(""); } }} />)}</div>)}</div> : <p className="profile-empty-copy">No career claims are recorded yet.</p>}<SourceJson title="Master CV source JSON" value={masterCv} /></section>
...
function ProfileClaimCard({ claim, canApprove, confirming, approving, error, onBeginApproval, onCancel, onConfirm }: { claim: any; canApprove: boolean; confirming: boolean; approving: boolean; error: string; onBeginApproval: () => void; onCancel: () => void; onConfirm: () => Promise<void> }) {
  return <article className="claim-card"><div className="claim-card-top"><div>{claim.role ? <strong>{claim.role}</strong> : null}{claim.organization ? <span>{claim.organization}</span> : null}</div><span className={claim.approved_for_tailoring ? "tailoring-status approved" : "tailoring-status blocked"}>{claim.approved_for_tailoring ? "Approved for tailoring" : "Not approved for tailoring"}</span></div><p>{claim.statement}</p>{claim.start_date || claim.end_date ? <small>{[claim.start_date, claim.end_date || "Present"].filter(Boolean).join(" – ")}</small> : null}{Array.isArray(claim.tags) && claim.tags.length ? <div className="tag-row">{claim.tags.map((tag: string) => <span key={tag}>{tag}</span>)}</div> : null}<div className="claim-card-footer"><Evidence provenance={claim.provenance} />{canApprove && !claim.approved_for_tailoring ? <button className="claim-approve-button" onClick={onBeginApproval}>Approve for tailoring</button> : null}</div>{confirming ? <div className="claim-approval-confirm"><strong>Confirm this claim is accurate</strong><p>Your confirmation will be recorded as evidence and a new approved profile version will be created.</p>{error ? <InlineError message={error} /> : null}<div><button className="text-button" disabled={approving} onClick={onCancel}>Cancel</button><button className="primary-button" disabled={approving} onClick={() => void onConfirm()}>{approving ? "Approving…" : "Yes, approve this claim"}</button></div></div> : null}</article>;
}
