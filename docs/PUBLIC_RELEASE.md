...
- A full-object audit of all reachable objects found no owner identifiers, no private document paths, and no excluded paths; the clone head tree is identical to the reviewed source HEAD tree (evidence: `artifacts/public-release/rewritten-audit.json`).
- Gitleaks 8.30.1 found no secrets in all rewritten refs (evidence: `artifacts/public-release/gitleaks-after.json`).
